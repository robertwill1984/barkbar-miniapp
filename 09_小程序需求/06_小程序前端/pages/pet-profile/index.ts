...
import { createPet, ensureCurrentUser } from '../../utils/api'
...
Page({
  ...
  onBreedInput(event: any) {
    this.setData({ 'pet.breed': event.detail.value })
  },
  onBehaviorInput(event: any) {
  ...
  async savePet() {
    ...
    try {
      ...
      const backendPet = await createPet(user.id, this.data.pet)
      savePetProfile({
        ...this.data.pet,
