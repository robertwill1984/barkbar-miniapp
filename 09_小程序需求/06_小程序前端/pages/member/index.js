const { memberCards } = require('../../utils/mock-data')
...
Page({
  ...
  async loadCards() {
    ...
    try {
      ...
      const cards = await api.listMemberCards(user.id)
      this.setData({
  ...
  async loadProducts() {
    try {
      const products = await api.listProducts('card')
  ...
  async loadOrders() {
    try {
      const user = await api.ensureCurrentUser()
      const orders = await api.listOrders(user.id, 'card')
      let refunds = []
      try {
        refunds = await api.listRefunds(user.id)
      } catch (error) {
        refunds = []
      }
      this.setData({
        orders: orders.map((item) => normalizeOrder(item, findRefundBySource(refunds, item.id))),
        refunds
      })
    } catch (error) {
  ...
  async buyProduct(event) {
    ...
    wx.showModal({
      ...
      success: async (modal) => {
        ...
        try {
          const user = await api.ensureCurrentUser()
          const orderResult = await api.createOrder(user.id, 'card', [{ sku_id: skuId, quantity: 1 }])
          let paymentStatus = orderResult.status === 'PAID' ? 'SUCCESS' : 'PENDING'
          try {
            const payment = await api.getOrderPaymentStatus(orderResult.order_id, user.id)
  ...
  requestRefund(event) {
    ...
    wx.showModal({
      ...
      success: async (modal) => {
        ...
        try {
          const user = await api.ensureCurrentUser()
          const refund = await api.createRefund(user.id, 'order', orderId, '用户在会员中心提交本地退款申请')
  ...
  async refreshRefundStatus(event) {
  ...
  async refreshRefundStatusById(refundId, userId, reloadOrders) {
    const refund = await api.getRefundStatus(refundId, userId)
  ...
  async generateCode(event) {
    ...
    try {
      const user = await api.ensureCurrentUser()
      const result = await api.generateMemberRedemptionCode(cardId, user.id)
      wx.hideLoading()
      this.setData({
        redemptionCode: {
          code: result.code,
...
function normalizeOrder(item, refund) {
...
function findRefundBySource(refunds, sourceId) {
  return (refunds || []).find((item) => item.source_id === sourceId || item.sourceId === sourceId)
}

function normalizeMockCard(item) {
  return {
    id: '',
    type: item.type,
    price: item.price,
...
function normalizeBackendCard(item) {
...
function mapCardPrice(cardType) {
...
function buildProductDesc(item) {
