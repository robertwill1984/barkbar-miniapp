const { reservationSlots, storeConfig } = require('../../utils/mock-data')
const {
...
Page({
  ...
  onShow() {
    ...
    this.setData({
      petName: pet && pet.name ? pet.name : '未填写',
  ...
  async loadRemoteSlots() {
    try {
      const slots = await api.getReservationSlots(this.data.selectedDate)
      this.setData({
  ...
  async submitReservation() {
    ...
    const slot = this.data.slots.find((item) => item.id === this.data.selectedSlotId) || this.data.slots[0]
    const requiresReview = pet.firstVisit || pet.firstSwim || Boolean(pet.behaviorNotes)
    wx.showLoading({ title: '提交中' })
    try {
      const user = await api.ensureCurrentUser()
      if (!pet.backendId) {
        const backendPet = await api.createPet(user.id, pet)
        pet = {
          ...pet,
      ...
      const remoteReservation = await api.createReservation(
        user.id,
        slot.id,
